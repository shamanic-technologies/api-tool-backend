...
import { OpenAPIObject } from 'openapi3-ts/oas30'; // Added: OpenAPIObject import for function signature
...
export const addNewTool = async (toolCreationData: CreateApiToolData): Promise<ApiTool> => {
    ...
    const toolDataForDb: Omit<ApiToolRecord, 'id' | 'created_at' | 'updated_at'> & { embedding?: number[] } = {
        ...
        is_verified: toolCreationData.isVerified === undefined ? false : toolCreationData.isVerified,
    ...
    try {
        ...
        return mapApiToolRecordToApiTool(createdToolRecord);
    } catch (error) {
